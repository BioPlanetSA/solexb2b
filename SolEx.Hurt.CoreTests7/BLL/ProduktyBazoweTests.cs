...
namespace SolEx.Hurt.CoreTests.BLL
{
    public class ProduktyBazoweTests
    {
        ...
        public void PasujeDoSzukaniaTest()
        {
            ...
            var accesor = typeof(ProduktBazowy).PobierzRefleksja();

            foreach (ProduktBazowy bazowy in produkty)
            {
                szukanaFraza = bazowy.KodKreskowy;
                Assert.True(pb.PasujeDoSzukania(bazowy, szukanaFraza,pola, accesor));
            }
            szukanaFraza = "5999";
            bool wynik = false;
            foreach (ProduktBazowy bazowy in produkty)
            {
                if (pb.PasujeDoSzukania(bazowy, szukanaFraza, pola, accesor))
                {
                    wynik = true;
                }
            }
        ...
        //    List<string> pola = "nazwa;kod;kod_kreskowy;KategorieTekstowo;pole_tekst1;opis".Split(';').ToList();

        //    A.CallTo(() => config.ProduktyWyszukiwanie).Returns(pola);
        //    Stopwatch stoper = Stopwatch.StartNew();
        //    SolexBllCalosc.PobierzInstancje.Szukanie.WyszukajObiekty(produkty, szukanaFraza, pola);
        ...
        public void PobierzWszystkieCechyProduktuWydajnoscTest()
        {
            ...
            string separator = ":";
            IConfigBLL config = A.Fake<IConfigBLL>();
            A.CallTo(() => config.SeparatorAtrybutowWCechach).Returns(separator.ToCharArray());
            int produkt = 1;
            List<ProduktCecha> cp;
            List<Cecha> cechy = DaneTestPobierania(out cp, separator, liczacech);
        ...
        [Fact(DisplayName = "Test pobierania stanów dla magazynów")]
        public void PobierzStanTest()
        {
            ISolexBllCalosc calosc = A.Fake<ISolexBllCalosc>();
            IProduktyStanBll stany = A.Fake<IProduktyStanBll>();
            A.CallTo(() => calosc.ProduktyStanBll).Returns(stany);

            Dictionary<int, Dictionary<long, decimal>> wszystkieStany = new Dictionary<int, Dictionary<long, decimal>>();
            wszystkieStany.Add(1, new Dictionary<long, decimal>() {{1, 1}, {2, 2}});
            wszystkieStany.Add(2, new Dictionary<long, decimal>() {{1, 10}, {2, 20}});
            wszystkieStany.Add(3, new Dictionary<long, decimal>() {{1, 100}, {2, 200}});
            A.CallTo(() => stany.WszystkieStanyDlaMagazynow()).Returns(wszystkieStany);

            ProduktBazowy pb1 = new ProduktBazowy
            {
                Id = 1,
                Calosc = calosc
            };

            decimal stan = pb1.PobierzStan(new HashSet<int>() {1});
            Assert.True(stan == 1);
            stan = pb1.PobierzStan(new HashSet<int>() {1, 2});
            Assert.True(stan == 11);
            stan = pb1.PobierzStan(new HashSet<int>() {1, 2, 3});
            Assert.True(stan == 111);
            stan = pb1.PobierzStan((HashSet<int>) null);
            Assert.True(stan == 111);

            bool blad = false;
            try
            {
                stan = pb1.PobierzStan(new HashSet<int>() {4});
            }
            catch
            {
                blad = true;
            ...
            ProduktBazowy pb2 = new ProduktBazowy
            {
                Id = 2,
                Calosc = calosc
            };

            stan = pb2.PobierzStan(new HashSet<int>() {1});
            Assert.True(stan == 2);
            stan = pb2.PobierzStan(new HashSet<int>() {1, 2});
            Assert.True(stan == 22);
            stan = pb2.PobierzStan(new HashSet<int>() {1, 2, 3});
            Assert.True(stan == 222);
            stan = pb2.PobierzStan((HashSet<int>) null);
            Assert.True(stan == 222);

            IKlient klient =A.Fake<IKlient>();
            klient.Id = 99;

            ProduktKlienta pk1 = new ProduktKlienta(pb1, klient) {Calosc = calosc};
            stan = pk1.PobierzStan(new HashSet<int>() { 1 });
            Assert.True(stan == 1);
            stan = pk1.PobierzStan(new HashSet<int>() { 1,2 });
            Assert.True(stan == 11);
            stan = pk1.PobierzStan(new HashSet<int>() { 1,2,3 });
            Assert.True(stan == 111);
            stan = pk1.PobierzStan((HashSet<int>)null);
            Assert.True(stan == 111);

            ProduktKlientaWirtualny pw1 = new ProduktKlientaWirtualny(pb1, klient, 99) {Calosc = calosc};
            stan = pw1.PobierzStan(new HashSet<int>() { 1 });
            Assert.True(stan == 1);
            stan = pw1.PobierzStan(new HashSet<int>() { 1, 2 });
            Assert.True(stan == 11);
            stan = pw1.PobierzStan(new HashSet<int>() { 1, 2, 3 });
            Assert.True(stan == 111);
            stan = pw1.PobierzStan((HashSet<int>)null);
            Assert.True(stan == 111);
        }
    }
}