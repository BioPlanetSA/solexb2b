...
if ( ! empty($_FILES) || isset($_POST['url']))
{
	...
	if (in_array(fix_strtolower($extension), $ext))
	{
		...
		if ($is_img)
		{
			...
			{
				$memory_error = TRUE;
				$memory_error_tmp ="B³ad tworzenia miniaturki obrazu. SprawdŸ uprawnienia. ";
			}
			else
			{
				...
				{
					$memory_error = TRUE;
					$memory_error_tmp ="B³ad tworzenia miniaturki. SprawdŸ uprawnienia. ";
				}
				else
				{
			...
			{
				unlink($targetFile);
				response(trans($memory_error_tmp."Not enought Memory").AddErrorLocation(), 406)->send();
				exit();
			}
		}
