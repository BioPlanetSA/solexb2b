...
begin
	...
	if exists ( SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID('FK_Klient_PoziomCenowy') AND parent_object_id = OBJECT_ID('Klient')) ALTER TABLE Klient DROP CONSTRAINT FK_Klient_PoziomCenowy;


	ALTER TABLE [dbo].Klient  WITH NOCHECK ADD  CONSTRAINT FK_Klient_PoziomCenowy FOREIGN KEY([PoziomCenowyId]) REFERENCES [dbo].[PoziomCenowy] ([Id]) ON DELETE SET NULL
