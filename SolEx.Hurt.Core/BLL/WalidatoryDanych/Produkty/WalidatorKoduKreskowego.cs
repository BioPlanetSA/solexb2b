...
namespace SolEx.Hurt.Core.BLL.WalidatoryDanych.Produkty
{
    /// <summary>
    /// Waliduje czy kod kreskowy jest poprawny - unikalny lub pusty
    /// </summary>
    public class WalidatorKoduKreskowego : IWalidatorDanych
    {
        /// <summary>
        ...
        public WalidacaDanychWynik Waliduj(string identyfikatorObiektu, string fraza)
        {
            ...
            {
                var produkty = SolexBllCalosc.PobierzInstancje.DostepDane.Pobierz<ProduktBazowy>(null, x => !string.IsNullOrEmpty(x.KodKreskowy) && x.KodKreskowy.Equals(testowanaFraza, StringComparison.InvariantCultureIgnoreCase)).ToList();
            ...
            if (!wynik)
            {
                komunikat = "Jest już produkt z takim kodem";
            }
