...
namespace ServiceStack.OrmLite
{
    public static class OrmLiteWriteExtensions
    {
        ...
        internal static void SaveAll<T>(this IDbCommand dbCmd, IEnumerable<T> objs)
        {
            ...
            var idMap = defaultIdValue != null ? saveRows.Where(x => !defaultIdValue.Equals(x.GetId())).ToDictionary(x => x.GetId()) : saveRows.Where(x => x.GetId() != null).ToDictionary(x => x.GetId());

            var existingRowsMap = dbCmd.GetByIds<T>(idMap.Keys).ToDictionary(x => x.GetId());

            //BARTEK: property probla pobrania
            PropertyInfo idPole = typeof(T).GetProperty("Id");
            //sprawedzam czu ID jest intem czy longiem 
            bool czyInt = typeof(T).GetInterfaces().Contains(typeof(IHasIntId));
            if (!idPole.CanWrite)
            {
                idPole = null;
            }
            ...
            foreach (var saveRow in saveRows)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        //bartek modyfiakcja zeby odrazu Id nadawac ZAWSZE
                        if (idPole != null)
                        {
                            //GetLastInsertId zrwaca zawsze longa dlatego dopisana jest konwersja na inta jeœli trzeba SG
                            var lastID = dbCmd.GetLastInsertId();
                            //jesli lastId jest równe 0 to prawdopodobnie id nie jest autoikrementowane i nienadpisujmy id które nadaliœmy w systemie SG
                            if (lastID != 0)
                            {
                                if (czyInt)
                                {
                                    idPole.SetValue(saveRow, (int) lastID);
                                }
                                else
                                {
                                    idPole.SetValue(saveRow, lastID);
                                }
                            }
                        }
                    }
                ...
                {
                    //Log.Error(ex);  //bartek komentuje - ten log to log servicestacka a nie nasz i nie widzimy go u siebie - chyba ze wywalimy exception tak jak teraz zmieniam
                    //próba wydostania SQL ktory sie zwalil
                    string sqlZwalony = null;
                    try
                    {
                        sqlZwalony = OrmLiteConfig.DialectProvider.ToInsertRowStatement(dbCmd, saveRow);
                    } catch
                    {}

                    if (sqlZwalony != null)
                    {
                        throw new Exception($"B³ad w SQL dla zapytania: {sqlZwalony}. B³¹d: {ex.Message}");
                    }
