...
namespace ServiceStack.OrmLite
{
    public abstract class SqlExpressionVisitor<T>
    {
        ...
        public virtual SqlExpressionVisitor<T> ThenBy<TKey>(Func<T, TKey> keySelector)
        {
            sep = string.Empty;
            useFieldName = true;
            var property = keySelector.ToString();
            orderByProperties.Add(property + " ASC");
            BuildOrderByClauseInternal();
            return this;
        }

        public virtual SqlExpressionVisitor<T> ThenBy(string thenBy)
        {
            sep = string.Empty;
            useFieldName = true;
            var property = thenBy;
        ...
        public virtual SqlExpressionVisitor<T> OrderByDescending(string orderByDesc)
        {
            orderByProperties.Clear();
            //this.orderBy = orderByDesc + " DESC";
            orderByProperties.Add(orderByDesc + " DESC");
            BuildOrderByClauseInternal();
            return this;
        }
        ...
        public virtual SqlExpressionVisitor<T> ThenByDescending(string thenByDesc)
        {
            sep = string.Empty;
            useFieldName = true;
            var property = thenByDesc;
            orderByProperties.Add(property + " DESC");
            BuildOrderByClauseInternal();
            return this;
        }
        ...
        private string PobierzWartoscSubquery(MethodCallExpression m, object quotedColName, bool zaprzeczenie = false)
        {
        ...
        protected virtual object VisitSqlMethodCall(MethodCallExpression m)
        {
            ...
            switch (m.Method.Name)
            {
                ...
                case "InSubquery":
                    statement = PobierzWartoscSubquery(m, quotedColName);
                    break;
                case "NotInSubquery":
                    statement = PobierzWartoscSubquery(m, quotedColName,true);
                    break;
                case "Desc":
